...
- **Dependencies:** New deps and version bumps require approval (name, version, justification, why existing deps don't solve it). **Pre-approved, no review needed:** Dependabot patch/minor bumps to development-scope or transitive packages, which auto-merge on green CI. Production deps, majors, and any hand-authored dependency change still require approval. Lockfile drift from main without explanation is stop-and-report.
- **Advisory triage:** `npm audit --omit=dev --audit-level=moderate` is a blocking CI gate — a production advisory fails the build. Development-scope advisories are hygiene, not incidents: nothing outside the production tree reaches the shipped bundle, and this app has no server. Do not open interrupt-driven PRs for them; they clear on the monthly Dependabot pass. See `docs/decisions/0002-dependency-update-policy.md`.
